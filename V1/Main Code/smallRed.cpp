...
void stacker() {
  ...
  while(true) {
    ...
    if(claw_manual) {
      ...
      if(grab_) claw_value = GRABFF - claw_.Calculate(claw_min - 1150, p_Claw.value(analogUnits::range12bit), 0);
      else claw_value = -claw_.Calculate(claw_min-20,p_Claw.value(analogUnits::range12bit), 0);
      v_claw(claw_value);
    }
